...
occs.ad <- read_csv("data/occurrences/filtered_distichus_occurrences.csv")
occs.ab <- read_csv("data/occurrences/filtered_brevirostris_occurrences.csv")
```
...
for (k in 1:9){
    if (k == 8){
        ...
        df2 <- occs.ab[!is.na(over(SpatialPoints(occs.ab[, 2:3]), poly)),]
        ...
        write_delim(x = df4, file = paste0("niche-assessment/", "K", k, "_total_pts.csv"), # NEEDS TESTING
        ...
        # Subset occurrences that fall within spatial polygon made above
        df2 <- occs.ad[!is.na(over(SpatialPoints(occs.ad[, 2:3]), poly)), ]
        # Combine total points
        df3 <- as_tibble(rbind(pts@coords, df2[, 2:3]))
        clust <- rep(k, nrow(df3))
        df4 <- as_tibble(cbind(df3, clust))
            df4$clust <- as.factor(df4$clust)
        write_delim(x = df4, file = paste0("niche-assessment/", "K", k, "_total_pts.csv"),
                    delim = ",", col_names = TRUE)
            assign(paste0("K", k, "_total_pts"), df4)
    }
}
...
all_pts <- as_tibble(rbind(K1_total_pts, K2_total_pts, K3_total_pts, K5_total_pts,
                    K6_total_pts, K7_total_pts, K8_total_pts, K9_total_pts))
...
# points(K8$presence.points, col = "black", pch = 15)
