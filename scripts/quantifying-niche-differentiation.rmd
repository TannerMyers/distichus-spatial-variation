...
K2_alpha_poly[[1]] <- ms_filter_islands(input = K2_alpha_poly[[1]], min_area = 1500000000)
    K2_spat_tot <- SpatialPoints(K2_total_pts[, 1:2])
    K2_spat_th <- SpatialPoints(K2_thinned[, 1:2])
    crs(K2_spat_tot) <- "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0" 
    crs(K2_spat_th) <- "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"
K2_total_pts <- K2_total_pts[!is.na(over(K2_spat_tot, K2_alpha_poly[[1]])), ]
    write_delim(x = K2_total_pts, file = "niche-assessment/K_2_total_pts.csv", delim = ",", col_names = TRUE)
K2_thinned <- K2_thinned[!is.na(over(K2_spat_th, K2_alpha_poly[[1]])), ]
    write_delim(x = K2_thinned, file = "niche-assessment/K_2_thinned.csv", delim = ",", col_names = TRUE)
```
