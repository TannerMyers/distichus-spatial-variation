...
K2_thinned <- K2_thinned[!is.na(over(K2_spat_th, K2_alpha_poly[[1]])), ]
    write_delim(x = K2_thinned, file = "niche-assessment/K2_thinned.csv", delim = ",", col_names = TRUE)
...
env_values <- raster::extract(all_pts[, 1:2], x = env_imputed)
...
```{r}
# for (k in c(1:5)){             ## FOR-LOOP NEEDS FIXING TO RUN
...
K_12_id <- identity.test(species.1 = K1, species.2 = K2, nreps = 5, env = env2, type = "mx")
K_13_id <- identity.test(species.1 = K1, species.2 = K3, nreps = 5, env = env2, type = "mx")
K_24_id <- identity.test(species.1 = K2, species.2 = K4, nreps = 5, env = env2, type = "mx")
K_34_id <- identity.test(species.1 = K3, species.2 = K4, nreps = 5, env = env2, type = "mx")
K_35_id <- identity.test(species.1 = K3, species.2 = K5, nreps = 5, env = env2, type = "mx")
```